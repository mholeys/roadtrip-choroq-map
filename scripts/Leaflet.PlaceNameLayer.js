	...
	L.LayerGroup.PlaceNameLayer = L.LayerGroup.extend({
		...
		options: {
			'sort': function(a,b){
				...
				var d = (b.options.capital - a.options.capital);
				if(d) return d;
				// Sort by population
				if(a.options.pop < b.options.pop) return 1;
				else if(a.options.pop > b.options.pop) return -1;
				// Sort by ID
				...
				.then(function(d){
					...
					for(r = 0; r < rows.length; r++){
						...
						if(r == 0) head = cols;
							...
							if(typeof rows[r].capital!=="number") rows[r].capital = 0;
							if(typeof rows[r].pop!=="number") rows[r].pop = -1;
