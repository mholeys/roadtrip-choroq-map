L.Control.PinSearch = L.Control.extend({
  ...
  onAdd: function(map) {
    ...
    input.addEventListener('input', function() {
      ...
      var matches = self.labels.filter(function(data) {
        var label = data[1];
        var marker = data[0];
      ...
      if (layer instanceof L.Marker && layer.options.title) {
        ...
        layer.getAllChildMarkers().forEach((child) => {
          if (child instanceof L.Marker && child.options.title) {
            this.labels.push(child.options.title);
          }
        });
      }
    }, this);
  ...
  _onInputChange: function(event) {
    ...
    var term = input.value.toLowerCase();
    var matches = this.labels.filter(function(data) {
      var label = data[1];
      var marker = data[0];
      return label.toLowerCase().includes(term);
    });
  ...
  _onSearchItemClick: function(query) {
    ...
    if (this.options.focusOnMarker) {
        ...
        if (marker) {
            ...
            this._map.flyTo(marker.getLatLng(), zoom);
        } else {
            var polyline = this._findPolylineByTitle(query);
            if (polyline) {
                var zoom = this._map._zoom;
                if (this.options.openPopup) {
                    polyline.openPopup();
                ...
                this._map.flyTo(polyline.getCenter(), zoom);
            }
        }
    }
  },
  ...
  _findPolylineByTitle: function(title) {
    var polyline = null;
    this._map.eachLayer(function(layer) {
      if (layer instanceof L.Polyline && layer.options.title === title) {
        polyline = layer;
      }
    });
    return polyline;
